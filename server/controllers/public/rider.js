...
router.post("/rider-register", async (req, res) => {
  try {
    ...
    if (dupUser) {
      if (dupUser.isActive === false)
        return res
          .status(400)
          .json({ msg: "your email has already been used" });
    ...
    // await sendSms(phone,`visit this link to verify your phone http://localhost:5000/public/rider-phone-verify/${phoneToken}`)
    let rider = {
      fullName,
      phone,
      email,
    ...
    res.status(200).json({ msg: "rider registered sucessfuly" });
  } catch (error) {
    console.log(error);
...
router.post("/rider-forgotpass", async (req, res) => {
  try {
    ...
    let otpUrl = `http://localhost:5000/public/rider-setnewpassword`;
    await sendMail(
      email,
