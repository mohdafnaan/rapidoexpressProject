...
router.post("/user-register", async (req, res) => {
  try {
    ...
    if (dupUser) {
      if(dupUser.isActive == false) return res.status(400).json({mag : "your email was already been used"})
    ...
    await sendMail(
      email,
      "Rapido Verification",
      `visit this link to verifiy your e-mail http://localhost:5000/public/user-email-verify/:${emailToken}`
    );
    // await sendSms(
    //   phone,
    //   `visit this link to verify your phone http://localhost:5000/public/user-phone-verify/:${phoneToken}`
    // );
...
router.get("/user-email-verify/:token", async (req, res) => {
  try {
...
router.get("/user-phone-verify/:token", async (req, res) => {
  try {
    let token = req.params.token;
